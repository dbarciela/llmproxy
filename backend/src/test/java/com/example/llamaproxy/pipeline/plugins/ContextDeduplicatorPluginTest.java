...
class ContextDeduplicatorPluginTest {
    ...
    void setUp() {
        ...
        requestContext = new RequestContext(org.springframework.http.HttpMethod.POST, "http://localhost", new org.springframework.http.HttpHeaders(), "{}");
    }
    ...
    private void mockSettings(boolean enabled, int threshold) {
        ...
        when(settingsManager.getSettingsAs("context-deduplicator", ContextDeduplicatorPlugin.DeduplicatorSettings.class))
                .thenReturn(settings);
    }
    ...
    void testNoMessagesArray() {
        ...
        assertEquals(originalPayload, requestContext.getPayload(), "Payload should not change if there are no messages");
    }
    ...
    void testNoDeduplicationBelowThreshold() {
        ...
        String originalPayload = String.format("{\"messages\":[{\"role\":\"user\",\"content\":\"%s\"}, {\"role\":\"user\",\"content\":\"%s\"}]}", history, current);
        requestContext.setPayload(originalPayload);

        plugin.processRequest(requestContext);

        assertEquals(originalPayload, requestContext.getPayload(), "Payload should not change if duplicated text is below threshold");
    }
    ...
    void testMultipleBlocksInOneMessage() {
        ...
        String newPayload = requestContext.getPayload();
        assertTrue(newPayload.contains("115 chars") || newPayload.contains("118 chars"), "First block not replaced: " + newPayload);
        assertTrue(newPayload.contains("125 chars") || newPayload.contains("128 chars"), "Second block not replaced: " + newPayload);
    }
    ...
    void testOverlapEdgeCase() {
        ...
        String block = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"; // 100 As
