...
public class ArchivePlugin implements AsyncPlugin {
    ...
    public ArchivePlugin(ProxySettings settings, SessionHistoryRepository repository, io.github.dbarciela.aura.pipeline.NotificationService notificationService) {
        ...
        if (!"archive".equals(notificationService.getActiveTab())) {
            if (!notificationService.hasUnreadNotification("archive")) {
                ...
                n.setActions(java.util.List.of(
                    new io.github.dbarciela.aura.pipeline.NotificationDTO.NotificationAction("View Log", null, null, "archive")
                ));
